Public Class CameraCapture
    ...
    Public Function CaptureAsBase64String() As String

        Dim CustomerPicture As String = ""
        ...
        Try
            ...
            If SendMessage(CamHwnd, WM_CAP_DRIVER_CONNECT, 0, 0) Then
                ...
                data = Clipboard.GetDataObject()
                If data.GetDataPresent(GetType(System.Drawing.Bitmap)) Then
                    Dim CustomerPictureBitmap As Bitmap = CType(data.GetData(GetType(System.Drawing.Bitmap)), Bitmap)
                    Clipboard.SetDataObject("N/A")

                    Dim ms As IO.MemoryStream = New IO.MemoryStream()
                    SaveJpegWithQuality(CustomerPictureBitmap, ms, 70) ' Adjust the quality level (0-100)
                    CustomerPicture = Convert.ToBase64String(ms.GetBuffer())
                    ms.Close()
    ...
    Private Sub SaveJpegWithQuality(ByVal bmp As Bitmap, ByVal ms As IO.MemoryStream, ByVal quality As Long)
        Dim encoderParameters As New Imaging.EncoderParameters(1)
        encoderParameters.Param(0) = New Imaging.EncoderParameter(Imaging.Encoder.Quality, quality)
        Dim jpegCodec As Imaging.ImageCodecInfo = GetEncoderInfo("image/jpeg")
        bmp.Save(ms, jpegCodec, encoderParameters)
    End Sub

    Private Function GetEncoderInfo(ByVal mimeType As String) As Imaging.ImageCodecInfo
        For Each codec As Imaging.ImageCodecInfo In Imaging.ImageCodecInfo.GetImageEncoders()
